...
import type { LayoutObject } from '../node/LayoutObject';
...
import type { CSSKeywordValue } from '../style/styleValue/keywordValue';
...
export class LayoutEngine {
  ...
  protected getLayoutDefinitionName(node: LayoutObject) {
    const layoutStyle = node.getAllStyle().get<CSSKeywordValue>(PropertyName.LAYOUT);
    if (!layoutStyle) {
  ...
  protected getNodeIntrisicSizes(node: LayoutObject): IntrinsicSizes {
    ...
    const minWidth =
      styleMap.get<CSSUnitValue>(PropertyName.MIN_WIDTH)?.value ||
      node.intrisicSizes?.minContentInlineSize ||
      styleMap.get<CSSUnitValue>(PropertyName.WIDTH)?.value ||
      0;
    const maxWidth =
      styleMap.get<CSSUnitValue>(PropertyName.MAX_WIDTH)?.value ||
      node.intrisicSizes?.maxContentInlineSize ||
      styleMap.get<CSSUnitValue>(PropertyName.WIDTH)?.value ||
      0;
    const minHeight =
      styleMap.get<CSSUnitValue>(PropertyName.MIN_HEIGHT)?.value ||
      node.intrisicSizes?.minContentBlockSize ||
      styleMap.get<CSSUnitValue>(PropertyName.HEIGHT)?.value ||
      0;
    const maxHeight =
      styleMap.get<CSSUnitValue>(PropertyName.MAX_HEIGHT)?.value ||
  ...
  protected getNodeFragment(
    ...
    const width =
      ...
      node.getAllStyle().get<CSSUnitValue>(PropertyName.MIN_WIDTH)?.value ||
      node.getAllStyle().get<CSSUnitValue>(PropertyName.WIDTH)?.value ||
      0;
    const height =
      node.getComputedLayout()?.blockSize ||
      node.getAllStyle().get<CSSUnitValue>(PropertyName.MIN_HEIGHT)?.value ||
      node.getAllStyle().get<CSSUnitValue>(PropertyName.HEIGHT)?.value ||
      0;
    return {
      inlineSize: width,
      blockSize: height,
